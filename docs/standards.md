...
- 배포는 깃허브의 이 저장소를 새 서버가 받아 docker 로 띄운다(`docs/operations.md` 「새 서버 배포」).
  옛 배포(저장소를 PHP 웹 루트로 rsync)는 버렸다 — 다시 쓰지 않는다.
- 서버에는 이미지 두 벌(web · api)만 간다. 저장소 파일(문서·스크립트)은 공개 주소로 나가지 않는다.
