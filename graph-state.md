...
| V3. 전체 검증 | 🔶 브라우저·속도 끝, C1 뒤 마지막 검사 | | 부모 docker(R1 까지 합친 것): verify 331/0/1 · web 8종 · check-pattern 0 · **관리 16 화면 × 1280·390 = 32: 넘침 0·h1 1·화면 오류 0·콘솔 오류 0** · 변경 이력 되돌리기 브라우저 왕복(비전 설명 고침 → 공개 반영 → 이력에서 되돌리기 → 원래와 같음, 알림 「되돌렸습니다.」) · **공개 10 장 × 4(1280·390·움직임 줄이기·JS 끔) = 40: 넘침 0·h1 1·콘솔 0, JS 끔·움직임 줄이기에서 숨은 글 0** · TTFB(로컬 docker, 중앙값): 메인 23ms · 페이지 엔진 9~15ms · 게시판 13~14ms · 정적 안내 7ms | 결정: 페이지 글 no-store 그대로(추가 5~15ms). 배포 직후 옛 메뉴 문제는 사라짐 — 공개 장이 전부 동적, 미리 그리는 공개 장은 숨김 patent_old 하나. api 를 다른 장비로 옮기면 TTFB 를 다시 잰다 |
