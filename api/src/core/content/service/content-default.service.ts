...
import { DirectusRow, DirectusService } from '../../../common/directus/directus.service'
...
export class ContentDefaultService {
  ...
  async findPosts(options: {
    ...
    const and: unknown[] = [PUBLISHED, ...(options.board ? [{ board: { _eq: options.board } }] : [])]
    if (keyword) {
      and.push({
    ...
    if (options.startDate && DAY_RE.test(options.startDate)) and.push({ published_date: { _gte: options.startDate } })
    if (options.endDate && DAY_RE.test(options.endDate)) and.push({ published_date: { _lte: options.endDate } })

    const res = await this.directus.get<DirectusRow[]>('/items/posts', {
      filter: JSON.stringify({ _and: and }),
      fields: 'id,board,slug,published_date,publish_at,unpublish_at,thumbnail,is_pinned,translations.*',
      deep: JSON.stringify({
  ...
  async fetchFile(id: string): Promise<UpstreamFile> {
    ...
    if (!(await this.fileIsPublic(id))) throw new CommonError(ContentError.FILE_NOT_FOUND)
    ...
    if (!up.ok || !up.body) {
      ...
      throw new CommonError(up.status >= 500 ? ContentError.FILE_UPSTREAM_ERROR : ContentError.FILE_NOT_FOUND)
    }
  ...
  private flat(row: DirectusRow, language?: string): DirectusRow {
    const { translations, ...rest } = row as DirectusRow & { translations?: DirectusRow[] }
    const rows = Array.isArray(translations) ? translations : []
    const t = rows.find((r) => r['languages_code'] === language) ?? rows[0] ?? {}
  ...
  private attachments(rows: unknown): Attachment[] {
    ...
    return rows.flatMap((r) => {
      const f = (r as DirectusRow)?.['directus_files_id'] as DirectusRow | undefined
      const id = f?.['id']
      if (typeof id !== 'string' || !id) return []
      const name = (f?.['title'] as string) || (f?.['filename_download'] as string) || '첨부파일'
