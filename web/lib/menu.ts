// **2026-09-22 부터 이 파일은 예비이자 씨앗이다.** 메뉴는 관리 화면 「사이트 › 메뉴」에서 고치고,
// 화면은 lib/menu-cms.ts 의 getMenu() 로 읽는다. api 가 안 닿거나 비었을 때만 이 목록을 쓴다.
// db/migrations/0006-menu.sql 의 씨앗이 이 목록을 그대로 옮긴 것이다 — 여기를 고쳐도 사이트
// 메뉴는 안 바뀐다(예비만 바뀐다).
//
